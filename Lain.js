...
const initLainPet = () => {
  ...
  bubble.style =
    "position:absolute; background:white; border:2px solid black; padding:8px; border-radius:10px; font-family:monospace; font-size:12px; opacity:0; transition: opacity 0.5s; width:150px; text-align:center; z-index:10000; pointer-events:none;";
  ...
  function draw() {
    ...
    if (!state.eventActive) {
      let newSrc =
        state.mode === "walk"
          ? state.vx >= 0
            ? assets[state.outfit].right
            : assets[state.outfit].left
          : assets[state.outfit].idle;
